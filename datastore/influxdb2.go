...
func (db *InfluxDB2) Get(ctx context.Context, key string) ([]interface{}, error) {
	result, err := db.queryAPI.Query(ctx,
		fmt.Sprintf(`from(bucket:"%s")|>range(start:-1h) |> filter(fn: (r) => r._measurement == "%s")`, db.bucket, key))
	if err != nil {
